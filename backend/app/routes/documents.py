...
- assembly — תקנונים ותקנוני משנה, פרוטוקולי אסיפה כללית של הקיבוץ (הגוף העליון). ההנחה הדיפולטית עבור bylaw/sub_bylaw.
- committee — פרוטוקולי ועד הנהלה, החלטות ועד הנהלה. חפש "ועד הנהלה"/"וועד" בכותרת/גוף המסמך.
- ballot — החלטות קלפי, תוצאות הצבעה בקלפי. בדרך כלל מציין "קלפי" או "הצבעה בקלפי" בכותרת.
- sub_committee — ועדות משנה (ועדת ביקורת, ועדת שיוך, ועדת חינוך, ועדת בריאות וכו').
- external_law — חוקי מדינה, פקודת האגודות השיתופיות, תקנות רשם האגודות.
- external_ruling — פסיקות בית משפט, החלטות רשם האגודות המחייבות את הקיבוץ.
- contract — הסכמים עם צדדים שלישיים (ספקים, נותני שירות).
- legal_opinion — חוות דעת משפטית מעורך דין.
- report — דוחות מבקר, דוחות כספיים, סקירות (אינפורמטיביים, לא מחייבים).
- notice — חוזרים והודעות פנימיות לחברים.
- procedure — נוהל פנימי שאינו תקנון או החלטה פורמלית.
- budget — תקציב שנתי או תקציב פרויקט.
...
def _classify_system_prompt(_existing_folders: list[str]) -> str:
    """Classifier prompt. Folder assignment moved out to
    services/folder_classifier.pick_folder (bounded pick from
    tenant-curated FolderTaxonomy), so folder hints no longer belong in
    this prompt. Arg kept for signature compatibility during rollout."""
...
def classify_one_document(db: Session, doc: Document, *, force: bool = False) -> dict:
    ...
    try:
        ...
        # row for reviewer). See services/folder_classifier for rationale.
        from app.services.folder_classifier import pick_folder
